...
// ref: github.com/projectdiscovery/naabu/v2/pkg/runner/ports.go
...
func ParsePorts(options *Options) ([]int, error) {
	...
	if options.ScanPorts != "" {
		var err error
		switch strings.ToLower(options.ScanPorts) {
		case "-":
			fallthrough
		case "full": // If the user has specfied full ports, use them
			...
			ports, err = excludePorts(options, ports)
			if err != nil {
				return nil, fmt.Errorf("could not read ports: %s", err)
			}
		case "top100": // If the user has specfied 100, use them
		...
		case "top1000": // If the user has specfied 1000, use them
		...
		default:
			ports, err = parsePortsList(options.ScanPorts)
			if err != nil {
				return nil, fmt.Errorf("could not read ports: %s", err)
			}
			ports, err = excludePorts(options, ports)
